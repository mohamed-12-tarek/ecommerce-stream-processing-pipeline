...
def generate_cutoffs(df: DataFrame) -> DataFrame:
    ...
    if start >= end:
        raise ValueError("Not enough data to generate temporal training examples.")
...
def build_lifetime_features(df: DataFrame, cutoffs: DataFrame) -> DataFrame:
    historical = (
        df.alias("events")
        .crossJoin(F.broadcast(cutoffs).alias("cutoffs"))
        .filter(F.col("events.timestamp") < F.col("cutoffs.cutoff_time"))
    )

    return (
        historical
        .groupBy("cutoffs.cutoff_time", "events.user_id")
        .agg(
            ...
            F.countDistinct("session_id").alias("number_of_sessions"),
...
def build_recent_features(df: DataFrame, cutoffs: DataFrame) -> DataFrame:
    historical = (
        df.alias("events")
        .crossJoin(F.broadcast(cutoffs).alias("cutoffs"))
        .filter(F.col("events.timestamp") < F.col("cutoffs.cutoff_time"))
    ...
    return (
        ...
        .agg(
            ...
            F.sum(
                F.when(
                    F.col("events.timestamp") >= F.col("cutoffs.cutoff_time") - F.expr("INTERVAL 24 HOURS"),
                    F.col("events.is_product_view")
                ).otherwise(0)
            ...
            F.sum(
                F.when(
                    F.col("events.timestamp") >= F.col("cutoffs.cutoff_time") - F.expr("INTERVAL 1 HOUR"),
...
def build_recency_features(df: DataFrame, cutoffs: DataFrame) -> DataFrame:
    historical = (
        df.alias("events")
        .crossJoin(F.broadcast(cutoffs).alias("cutoffs"))
        .filter(F.col("events.timestamp") < F.col("cutoffs.cutoff_time"))
    ...
    return (
        ...
        .agg(
            ...
            ).alias("time_since_last_purchase_seconds"),
        )
    )


def build_target(df: DataFrame, cutoffs: DataFrame) -> DataFrame:
    future = (
        df.alias("events")
        .crossJoin(F.broadcast(cutoffs).alias("cutoffs"))
        .filter(
            (F.col("events.timestamp") >= F.col("cutoffs.cutoff_time"))
            &
            (F.col("events.timestamp") < F.col("cutoffs.cutoff_time") + F.expr("INTERVAL 1 HOUR"))
        )
        .filter(F.col("events.event_type") == "purchase")
    )

    return (
        future
        .groupBy("cutoffs.cutoff_time", "events.user_id")
        .agg(F.lit(1).alias("future_purchase"))
    )


def build_training_dataset(df: DataFrame) -> DataFrame:
    cutoffs = generate_cutoffs(df)
    lifetime = build_lifetime_features(df, cutoffs)
    recent = build_recent_features(df, cutoffs)
    recency = build_recency_features(df, cutoffs)
    target = build_target(df, cutoffs)

    dataset = (
        lifetime
        .join(recent, ["cutoff_time", "user_id"], "left")
        .join(recency, ["cutoff_time", "user_id"], "left")
        .join(target, ["cutoff_time", "user_id"], "left")
        .fillna(
            {
                "future_purchase": 0,
                "views_last_1h": 0,
    ...
    return (
        ...
        .withColumn(
            ...
            F.when(
                ...
                F.col("add_to_carts") / F.col("product_views"),
            ).otherwise(0.0),
        )
        .orderBy("cutoff_time", "user_id")
    )